...
- Action-capable clients use `src/krcn_core/application.py`. The `krcn project`, `krcn installation`, `krcn release`, `krcn deployment`, `krcn knowledge`, `krcn context-package`, and `krcn memory` commands are thin CLI adapters over the same service used by SDK, MCP, plugins, Codex, Claude, and other clients.
