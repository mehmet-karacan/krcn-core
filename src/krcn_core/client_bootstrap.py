...
- If a project is matched, run `krcn project integrate --project "<project-id>" --scan-mode automatic --format json`. A fresh complete integration is a no-op. When an already registered project needs only read-only source observation and local KRCN bookkeeping or derived-index reconciliation, continue with the same command plus `--apply` without asking for another approval. New registration and every source, policy, provider, database, Git, secret, cross-project, destructive, or non-bookkeeping mutation keep their normal exact-plan approval gate. An explicit project refresh uses manual scan mode so add, update, and delete changes in the source tree are observed immediately.
...
- After an already authorized execution has completed every checkpoint and independent verification has passed every bound acceptance criterion, allow the shared orchestration service to append its completion attestation and close the one active Work Graph item without asking for a second approval. Never use this path for manual or forced closure, missing proof, bulk or cross-project changes, deletion, or any new source, provider, database, Git, policy, secret, or capability authority. Repeating the same verified finish is a no-op; reopening remains a visible normal revision.
- Do not ask twice for the same unchanged authority. Routine KRCN-local checkpoint, progress, handoff, evidence, receipt, registered-project observation, knowledge, index, rebuild, and repair effects continue after their exact dry-run without another prompt. A human's current explicit local mutation request may be reused only through trusted host or interactive evidence bound to session, user turn, client request, operation, project, exact plan, targets, and effect digest; its consumed receipt is single-use and an exact replay returns the prior result. Never synthesize this evidence in an agent subprocess. Reviewed implementation delivery may reuse this authority only for at most 20 non-delete files already bound to its sandbox artifact, source revision, tests, and exact mutation plan. Ambiguous or expanded scope, delete/purge/bulk/cross-project work, inferred source changes, and provider, database, Git, deploy, secret, or cost effects keep their dedicated gate.
- In Codex, Claude Code, and OpenCode, route a user's exact natural work-item creation command through `krcn ask "<current user text>" --apply` with the real client and session identifiers. This shared path parses the current text server-side, creates the exact Work Graph plan, and applies it in the same request without a second prompt. Do not use it for inferred or rewritten requests.
