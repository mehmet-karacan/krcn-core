...
`project integrate` completes registration, discovery, evidence-bound knowledge, capability-profile, local knowledge-vector index, contentless source-code index, and verification stages. Manual mode always scans for an explicit refresh request, including source add, update, and delete observations. Automatic mode scans after the versioned 24-hour freshness interval or when a required stage is missing. A fresh complete integration is a no-op. For an already registered project, `--apply` may reconcile only reversible KRCN bookkeeping and rebuildable indexes without an approval id or a separately copied plan id. The same-call plan is still verified before writing. Registration, project-source mutation, policy, provider, database, Git, secret, cross-project, destructive, and other user-data effects preserve their existing exact-plan approval gates.
